...
def write_final_report(
    ...
    seller = advanced["seller"]
    customer = advanced["customer"]
    top_category = category["top_contributors"][0]
    top_state = region["top_contributors"][0]
    highest_late = region["highest_late_states"][0]
    ...
    lines = [
        ...
        "",
        (
            "本项目以Olist公开电商订单数据为对象，先建立可追溯的分析数据底座和指标口径，"
            "再完成经营增长、品类、地区履约、商家和用户留存分析。在此基础上，项目加入可控语义层Copilot、"
            "交互看板和50题离线评测，评估自然语言取数的准确性与安全性。"
        ),
        "",
        "## 核心结论",
        "",
        ...
        "|---|---|---|",
        (
            f"| 经营增长 | 2018年1至8月GMV同比 {overview['ytd_gmv_growth']:.1%}，"
            f"订单量同比 {overview['ytd_order_growth']:.1%}，客单价变化 "
            f"{overview['ytd_aov_growth']:.1%} | 增长主要由订单规模扩大驱动，不是客单价提升 |"
        ),
        (
            f"| 品类 | {str(top_category['product_category']).replace('_', ' ')}贡献净增量的 "
            f"{float(top_category['growth_contribution']):.1%}，前五品类合计 "
            f"{category['top_five_growth_contribution']:.1%} | 增量来源有所集中，但单一品类依赖不高 |"
        ),
        (
            f"| 地区 | {top_state['customer_state']}州贡献净增量的 "
            f"{float(top_state['growth_contribution']):.1%}，前五州合计 "
            f"{region['top_five_growth_contribution']:.1%} | 增长地域集中度高，地区策略应区分规模与履约风险 |"
        ),
        (
            f"| 履约 | {highest_late['customer_state']}州延迟率 "
            f"{float(highest_late['late_delivery_rate']):.1%}；州级延迟率与评分相关系数 "
            f"{region['late_review_correlation']:.2f} | 优先治理大规模、高延迟的州和商家 |"
        ),
        (
            f"| 商家 | Top 10商家GMV份额 {seller['top_10_share']:.1%}，HHI "
            f"{seller['hhi']:.4f} | 头部依赖较低，运营重点是长尾商家分层和履约治理 |"
        ),
        (
            f"| 用户 | 复购率 {customer['repeat_rate']:.1%}，M1留存 "
            f"{customer['m1_retention']:.2%} | 一次性购买占主导，优先验证首购后30天二购激励 |"
        ),
        (
            f"| Copilot | 50题中答案准确率 {semantic['answer_accuracy']:.1%}，"
            f"关键词基线 {baseline['answer_accuracy']:.1%}，安全拦截率 "
            f"{semantic['safety_accuracy']:.1%} | 语义层与受控SQL模板能提高回归集稳定性 |"
        ),
        "",
